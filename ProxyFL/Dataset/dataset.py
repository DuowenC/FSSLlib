...
def show_clients_data_ratio(dict_per_client_labeled, num_classes):
    per_client_labeled_ratio = []

    for client, indices in enumerate(dict_per_client_labeled):
        local_num_labeled = sum(indices)
        per_client_labeled_ratio.append([i / local_num_labeled for i in indices])
        logging.info(f'client {client} labeled ratio per class : {per_client_labeled_ratio[-1]}')
    return per_client_labeled_ratio
